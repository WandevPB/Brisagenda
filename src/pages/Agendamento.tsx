...
import { formatarDataBrasileira } from '@/lib/dateUtils';
...
const Agendamento = () => {
  ...
  const buscarHorariosDisponiveis = async (data: string, cd: string) => {
    setIsLoadingHorarios(true);
    
    try {
      const response = await agendamentoService.getHorariosDisponiveis(data, cd);
      
      if (response.horariosLivres && Array.isArray(response.horariosLivres)) {
        setHorariosDisponiveis(response.horariosLivres);
      } else {
      ...
      setHorariosDisponiveis([]);
      toast.error('Erro ao buscar horários disponíveis: ' + (error.response?.data?.error || error.message));
    } finally {
      setIsLoadingHorarios(false);
    }
  };

  // Atualizar horários quando mudar CD ou data
  React.useEffect(() => {
    if (formData.centroDistribuicao && formData.dataEntrega) {
      // Usar data local em vez de UTC para evitar problemas de fuso horário
      const ano = formData.dataEntrega.getFullYear();
      const mes = String(formData.dataEntrega.getMonth() + 1).padStart(2, '0');
      const dia = String(formData.dataEntrega.getDate()).padStart(2, '0');
      const data = `${ano}-${mes}-${dia}`;
      
      buscarHorariosDisponiveis(data, formData.centroDistribuicao);
    } else {
      setHorariosDisponiveis([]);
    }
  }, [formData.centroDistribuicao, formData.dataEntrega]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Preparar dados para envio - usar data local em vez de UTC
      const ano = formData.dataEntrega!.getFullYear();
      const mes = String(formData.dataEntrega!.getMonth() + 1).padStart(2, '0');
      const dia = String(formData.dataEntrega!.getDate()).padStart(2, '0');
      const dataEntregaLocal = `${ano}-${mes}-${dia}`;

      console.log('� CRIANDO AGENDAMENTO - Data selecionada (formatada):', formatarDataBrasileira(formData.dataEntrega!));
      console.log('� CRIANDO AGENDAMENTO - Data para banco:', dataEntregaLocal);
                ...
                <div className="grid md:grid-cols-2 gap-4">
                      ...
                      <PopoverTrigger asChild>
                        ...
                        >
                          ...
                          {formData.dataEntrega ? (
                            formatarDataBrasileira(formData.dataEntrega)
                          ) : (
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      disabled={!formData.centroDistribuicao || !formData.dataEntrega || isLoadingHorarios}
                    >
                      <SelectTrigger className="border-orange-200 focus:border-orange-500">
                        <SelectValue placeholder={
                          isLoadingHorarios 
                            ? "Carregando horários..." 
                            : !formData.centroDistribuicao || !formData.dataEntrega
                            ? "Selecione CD e data primeiro"
                            : "Selecione o horário"
                        } />
                      </SelectTrigger>
                      <SelectContent>
                        {horariosDisponiveis.length === 0 && !isLoadingHorarios && formData.dataEntrega && formData.centroDistribuicao && (
