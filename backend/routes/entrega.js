...
router.get('/hoje', async (req, res) => {
  try {
    ...
    // Usar data local correta - não usar offset
    ...
    console.log(`📅 DEBUG - Data hoje local: ${hoje}`);
    ...
    // Debug das entregas encontradas
    if (entregas.length > 0) {
      console.log('📋 Entregas encontradas:');
      entregas.forEach(entrega => {
        console.log(`  - ID: ${entrega.id}, Empresa: ${entrega.empresa}, Data: ${entrega.data_entrega}, Horário: ${entrega.horario_entrega}`);
      });
    }
    console.log(`✅ Encontradas ${entregas.length} entregas para hoje`);
...
router.get('/estatisticas', async (req, res) => {
  try {
    ...
    // Usar data local correta para calcular a data de início
    const agora = new Date();
    ...
    const queryStats = `
      SELECT 
        ...
        COUNT(CASE WHEN status_entrega IS NULL AND data_entrega < date() AND data_entrega >= ? THEN 1 END) as pendentes_confirmacao
      FROM agendamentos 
      WHERE centro_distribuicao = ? 
        AND status IN ('confirmado', 'sugestao_enviada')
    `;
